...
The same frozen analysis applied to the schedule/task sensitivity experiment gives the default-release results in \Cref{tab:app-metric-stage-e}.
...
\subsection{Primary architecture robustness analyses}\label{app:stage-f-primary}
...
\subsection{Architecture-specific estimates}\label{app:stage-f-architectures}

\Cref{tab:app-stage-f-all-architectures} reports every architecture mean and marginal 95\% $t$ interval for the two frozen primary metrics.

{\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{lrrrr}
\caption{\texttt{two\_concepts} architecture means for $B$.}\label{tab:app-stage-f-all-architectures}\\
\toprule
...
\subsection{Prespecified architecture-factor contrasts}\label{app:stage-f-factors}

\Cref{tab:app-stage-f-factors} gives all prespecified factor contrasts on the primary \texttt{two\_concepts} task. Holm adjustment is applied within metric, task, and factor family exactly as frozen in the architecture robustness protocol. The signs are alternative architecture minus reference architecture as named in each row.

\begin{landscape}
{\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{p{.10\linewidth}p{.38\linewidth}rrrr}
\caption{Architecture-factor contrasts.}\label{tab:app-stage-f-factors}\\
\toprule
...
The secondary task also shows substantial variation across architectures, but its sign pattern differs in several settings. \Cref{tab:app-stage-f-single} is included to make that task dependence visible rather than pooling the two tasks.
