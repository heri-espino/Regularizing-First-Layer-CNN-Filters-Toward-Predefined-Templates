...
On \texttt{two\_concepts}/TinyCNN, selected-patch counterfactual accuracy is 93.61\% under constant retention and 90.64\% under annealed even though $U_{\mathrm{random}}$ is higher under annealed. This discrepancy motivated separate reporting of selected intervention behavior and selected-versus-control scores.

\subsection{Stage-C decomposition of the original selected-minus-random score}\label{app:stage-c-decomposition}

Stage C reused the 80 final Stage-B annealed/retention checkpoints and evaluated three validation-only rankings at $k\in\{1,2,4,8\}$. The central TinyCNN result is reproduced below. Each entry is annealed minus constant retention; intervals are marginal paired 95\% $t$ intervals over ten blocks. Stage C is post hoc and supplies no confirmatory test.
