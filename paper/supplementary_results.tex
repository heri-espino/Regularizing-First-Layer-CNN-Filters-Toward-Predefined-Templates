...
Stages A--C precede the prespecified prospective Stage-D experiment reported in Appendix~\ref{app:confirmation}. They are retained to show why the prospective primary analysis ultimately used the patching effect for selected channels rather than the selected-minus-random comparison.
...
The table below gives the Stage-B epoch-200 quantities most relevant to the constant-versus-annealed regularization comparison. Means are over ten replicates. The full five-condition tables and checkpoint trajectories remain in the analysis artifact.
...
\subsection{Stage-C decomposition of the original selected-minus-random comparison}\label{app:stage-c-decomposition}

Stage C reused the 80 epoch-200 Stage-B checkpoints from the annealed- and constant-regularization conditions and evaluated three validation-only channel-selection rules at $k\in\{1,2,4,8\}$. The TinyCNN results are reproduced below. Each entry is annealed minus constant regularization; intervals are marginal paired 95\% $t$ intervals over ten replicates. Stage C is post hoc and is not used as the prospective primary test.
