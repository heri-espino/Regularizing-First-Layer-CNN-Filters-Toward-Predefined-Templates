...
\begin{table}[h]
\caption{Primary architecture-averaged selected-channel structured-minus-pixel-permuted channel-count contrast on \texttt{two\_concepts}. Difference tests use 100,000 sign-flip permutations and Holm correction across the two primary metrics. Equivalence uses the frozen TOST margins and 90\% confidence intervals.}
...
The equivalence margins were frozen before the fresh outcomes as 20\% of the historical mean absolute $B$ across the same four diagnostic architecture forms: $\delta_{\mathrm{clogit}}=0.01704$ and $\delta_{R_{\mathrm{prob}}}=0.06594$. These are study-specific, scale-relative smallest-effect conventions rather than externally validated thresholds of scientific importance. For centered logits, the architecture-averaged 90\% interval is $[0.05219,0.06266]$ and the Holm-adjusted TOST $p$ value is 1. For probability error reduction, the architecture-averaged 90\% interval is $[0.03864,0.05261]$ and the Holm-adjusted TOST $p$ value is $5.03\times10^{-6}$. Thus the architecture-averaged probability-error difference is statistically nonzero while falling inside the prespecified equivalence region. The smallest symmetric margin that would contain its observed 90\% interval is approximately $0.05261$, about 16.0\% of the historical scale used to define the frozen 20\% margin; this sensitivity is reported to make the role of the margin explicit.
...
\begin{table}[h]
\caption{Architecture-averaged structured-minus-pixel-permuted $B$ for random channel sets on \texttt{two\_concepts}. Both effects are statistically distinguishable from zero and fall within the frozen scale-relative equivalence margins.}
...
The random-channel spatial-structure interaction remains detectable but is smaller for centered logits: partial $\eta^2=0.0683$ with permutation $p=8.0\times10^{-5}$. For probability error reduction it remains substantial, partial $\eta^2=0.2910$ with permutation $p=1.0\times10^{-5}$. Thus small architecture-averaged random-channel effects coexist with architecture-dependent differences; the equivalence statements apply to the architecture averages, not to every architecture separately. These are prespecified robustness diagnostics, not a second primary family.
...
\begin{table}[h]
\caption{Exact architecture grid used in the prospectively frozen architecture study. Parameter counts include the classifier.}
