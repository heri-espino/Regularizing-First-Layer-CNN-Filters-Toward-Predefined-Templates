...
The revised main text uses the one-to-one matching analysis only as a compact structural robustness check. This appendix reports the complete epoch-200 numerical table; the archived research artifact also retains the matching trajectories, full similarity matrices, and matched-filter gallery from the earlier manuscript version.

All values below are epoch-200 means $\pm$ sample standard deviation across ten replicates; conditions are paired within replicate. The nearest-template and one-to-one scores use signed cosine similarity. Coverage counts distinct nearest-template entries; because the bank has rank 10, it is neither a count of linearly independent directions nor a count of independent semantic concepts. The analysis uses only saved checkpoints from the earlier analyses. These descriptive comparisons are exploratory.
