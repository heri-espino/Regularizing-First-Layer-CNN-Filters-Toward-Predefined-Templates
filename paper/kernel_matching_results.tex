...
The main Results section contains three visual summaries of this exploratory Stage-B follow-up: the matching trajectories, the complete similarity matrices for one paired replicate, and the matched filter gallery. This appendix keeps only the complete epoch-200 numerical table so that the same visual evidence is not repeated at the end of the paper.

All values below are epoch-200 means $\pm$ sample standard deviation across ten paired replicates. The nearest-template and one-to-one scores use signed cosine similarity. Coverage counts distinct nearest-template identities, not independent semantic concepts. The analysis uses only saved Stage-B checkpoints. These descriptive comparisons are exploratory.
