...
\hypersetup{pdfauthor={},pdftitle={Activation-Patching Comparisons in Controlled CNNs Depend on Channel Budget, Architecture, and Spatial Prior Structure}}

\title{Activation-Patching Comparisons in Controlled CNNs Depend on Channel Budget, Architecture, and Spatial Prior Structure}
\author{}
\date{}
...
\begin{abstract}
Activation patching asks how a model output changes when internal activations are replaced, but the resulting measurement is conditional on how the intervention is defined. We study this problem in controlled convolutional networks whose first layer is initialized from a predefined bank of edge, corner, and ring filters. Constant regularization preserves this spatial structure, whereas an annealed condition releases the first layer during training. On exact matched counterfactual image pairs, a prospectively frozen 20-block test shows that the release-versus-retention comparison changes sharply with the number of patched first-layer channels; the same prespecified small-versus-larger-channel contrast remains positive under centered-logit fidelity and unnormalized probability-error reduction, although the pointwise curves differ. A separate 100-block study across 16 CNN variants finds strong architecture heterogeneity under both primary metrics, and post hoc Greenhouse--Geisser, permutation, Friedman, and random-channel diagnostics preserve that conclusion. Finally, a fresh 100-block matched-anchor study compares the original filters with a common pixel permutation that preserves the template-bank Gram matrix, rank, singular spectrum, row norms, and coefficient multisets. For validation-selected channels, the architecture-averaged centered-logit difference is non-equivalent under a prespecified scale-relative margin, whereas the architecture-averaged probability-error difference is statistically nonzero but falls within its prespecified margin. The spatial effect reverses sign in one downstream architecture and is substantially smaller on average for random channel sets. Thus weight-space structure, concentration of functional effects in selected channels, and activation-patching measurements are distinct quantities: conclusions in this setting depend on channel budget, downstream architecture, output metric, and spatial prior structure.\footnote{ChatGPT (OpenAI) was used as an assistive tool during experimental planning, code development and debugging, manuscript organization, and language editing. All experimental protocols, analyses, interpretations, and manuscript text were reviewed by the authors, who take full responsibility for the work.}
\end{abstract}
...
Matched image pairs are repeated measurements within a model/block and are never treated as independent inferential replicates. The evidence was accumulated sequentially rather than specified as one joint preregistration. Earlier developmental analyses motivated a prospectively frozen 20-block \texttt{two\_concepts} channel-budget test in TinyCNN. Its architecture contrast and the frozen alternative-metric reanalysis then motivated a separate 100-block architecture study with 16 downstream variants. After that architecture map was observed, it motivated the fresh matched-anchor question and informed the balanced four-architecture diagnostic subset used in a further 100 fresh blocks. Each follow-up protocol, its estimands, and its fresh renderer blocks were frozen before outcomes from that follow-up were inspected; the follow-ups are therefore prospective with respect to their own fresh outcomes but outcome-informed by the preceding studies.

The 16-architecture and matched-anchor studies each average four paired initialization or init/anchor replicates within renderer block before inference. Their inferential sample size is therefore $n=100$ renderer blocks, not the 25,600 trained models in each grid. Alternative-metric reanalysis of saved prospective checkpoints and the later Greenhouse--Geisser, permutation, Friedman, and random-channel architecture diagnostics are explicitly post hoc with respect to the original outcomes, although their analysis definitions were frozen before recomputation. Complete developmental and secondary schedule analyses are retained in the appendix but are not pooled with fresh-sample tests.
...
Averaged over the four diagnostic architectures on \texttt{two\_concepts}, selected-channel centered-logit $B$ differs between structured and pixel-permuted anchors by $+0.05743$ (95\% CI $[+0.05116,+0.06369]$, Holm sign-flip $p=2.0\times10^{-5}$). Before these outcomes were inspected, the equivalence margin was set to 20\% of the historical mean absolute $B$ across the same four architecture forms. The centered-logit 90\% interval $[+0.05219,+0.06266]$ lies entirely outside that scale-relative margin ($\pm0.01704$), so the architecture-averaged effect is statistically distinguishable from zero and not equivalent under the prespecified rule.

For probability error reduction, the architecture-averaged difference is $+0.04563$ (95\% CI $[+0.03728,+0.05397]$, Holm sign-flip $p=2.0\times10^{-5}$). Its 90\% interval $[+0.03864,+0.05261]$ lies inside the wider prespecified scale-relative margin ($\pm0.06594$). Thus the architecture-averaged probability-error contrast is precisely nonzero but falls within the frozen equivalence region. This equivalence statement is tied to that prespecified historical-scale margin; it is not a claim that every architecture has a negligible spatial-structure effect.
...
\caption{Fresh matched-anchor specificity experiment. \textbf{(a)} Architecture-specific selected-channel difference in $B$ between the original structured bank and its common pixel-permuted control on \texttt{two\_concepts}. The spatial effect reverses sign in \texttt{plain2\_w16\_gmp}. Bars are paired 95\% $t$ intervals over 100 fresh renderer blocks. \textbf{(b)} Architecture-averaged structured-minus-pixel-permuted effects for validation-selected and random channel sets. Bars are 90\% intervals used by the frozen TOST procedure; shaded regions denote the prespecified 20\%-of-historical-scale equivalence margins. The selected-channel centered-logit average is not equivalent, whereas the selected-channel probability-error average and both random-channel averages fall within their respective frozen margins.}
\label{fig:anchor-specificity}
\end{figure}

The random-channel diagnostic constrains the interpretation further. Averaged across architectures, structured-minus-pixel-permuted random-channel $B$ is $+0.00571$ for centered logits and $+0.02007$ for probability error reduction, and both architecture-averaged effects fall within the frozen equivalence margins. These averages are substantially smaller than the corresponding selected-channel centered-logit effect, although architecture-dependent random-channel differences remain detectable. Spatial specificity is therefore stronger on average for the validation-selected channel analysis, not absent for arbitrary same-size channel subsets.
...
The matched-anchor experiment sharpens that conclusion. The original spatial arrangement is not merely decorative: under centered-logit fidelity, architecture-averaged selected-channel $B$ differs clearly from a control that preserves the bank's complete non-spatial channel geometry. At the same time, this is not a universal template benefit. The effect reverses in one architecture; the architecture-averaged probability-error contrast falls within the prespecified scale-relative equivalence margin; and the architecture-averaged random-channel contrasts are smaller. The most defensible interpretation is therefore that designed spatial structure changes where the release-versus-retention functional difference concentrates, conditional on the downstream computation, output metric, and channel-selection rule.
...
\item The architecture-averaged anchor-specificity probability-error difference is statistically nonzero but falls within the frozen 20\%-of-historical-scale equivalence margin. That margin was prespecified before the fresh matched-anchor outcomes, but it is a scale-relative study convention rather than an externally validated threshold of scientific importance; architecture-specific effects can be larger and can change sign.
\item The experimental program is sequential. The architecture study was motivated by earlier outcomes, and the matched-anchor task/architecture subset was chosen after inspecting the architecture map. Each follow-up used a frozen protocol and fresh renderer blocks before its own outcomes were seen, but the sequence should not be interpreted as one jointly preregistered experiment.
...
Preserving a predefined first-layer spatial basis does not yield a single activation-patching signature. In controlled CNNs with exact matched counterfactuals, the release-versus-retention comparison changes with the number of patched channels and varies strongly across downstream architectures. Those conclusions survive alternative output metrics and sphericity-robust, permutation-based, and random-channel diagnostics. A fresh matched-anchor experiment further shows that the original two-dimensional edge/corner/ring arrangement changes selected-channel patch-budget behavior beyond the bank's Gram matrix, rank, and singular spectrum---but the effect is architecture-dependent, metric-dependent, and smaller on average for random channel sets. Structural retention, selected-channel functional concentration, and intervention-based measurement should therefore be treated as distinct empirical quantities.
