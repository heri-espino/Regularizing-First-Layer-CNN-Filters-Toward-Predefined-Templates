"""Evaluate frozen Stage-F checkpoints over all first-layer patch budgets."""
...
@torch.inference_mode()
def evaluate_one(model, val, test, task, arch, block, init_rep, batch, device):
    hv, lv = collect(model, val["x"], batch, device)
    ht, lt = collect(model, test["x"], batch, device)

    # Frozen validation-only contrast ranking. The ranking statistic is the same
    # first-layer global-max summary used in earlier stages, independent of the
    # architecture's downstream pooling choice.
    zv = hv.amax((2, 3)).cpu()
    ...
    rr = ac.control_rng(block, task, arch, init_rep)
    random_orders = [
    ...
    matching = []
    for k in KS:
        selected_logits = patch_logits(model, ht, ranking, k, pairs, batch)
        selected = measure(selected_logits, logits_0, logits_1, labels)
        ...
        for metric in CONTROL_METRICS:
            ...
            row[f"selected_minus_random_{metric}"] = (
                None if selected[metric] is None or rv is None else selected[metric] - rv
            )
        ...
        if k in MATCH_KS:
            ...
            for metric in CONTROL_METRICS:
                ...
                row[f"selected_minus_energy_{metric}"] = (
                    None if selected[metric] is None or ev is None else selected[metric] - ev
                )
        rows.append(row)
    ...
    return {
        ...
        "prob_input_effect_mpp": base["prob_error_reduction"] * 0.0 + squared_error_mpp(logits_0.softmax(1), logits_1.softmax(1)),
...
def main():
    ...
    if not (inp / "GRID_COMPLETE.json").exists():
        raise SystemExit("Stage-F training grid is not marked complete.")

    expected = (
        int(design["blocks"])
        * int(design["init_reps"])
        * len(design["tasks"])
    ...
    eval_design = {
        "study": "cnn_architecture_robustness_stage_f_evaluation",
        ...
        "ranking": "validation_standardized_activation_contrast",
    ...
    if design_path.exists():
        if json.loads(design_path.read_text()) != eval_design:
            raise SystemExit("Existing Stage-F evaluation design differs. Use a new output root.")
    else:
    ...
    for cp in sorted((inp / "runs").glob("*/*/block*/init*/*/epoch_*.pt")):
        ...
        arch = cp.parent.parent.parent.parent.name
        task = cp.parent.parent.parent.parent.parent.name
        jobs.append((task, arch, block, init_rep, treatment, cp))

    if len(jobs) != expected:
        raise SystemExit(f"Expected {expected} Stage-F checkpoints, found {len(jobs)}")
    if args.smoke:
    ...
    for i, (task, arch, block, init_rep, treatment, cp) in enumerate(jobs, 1):
        target = (
            out
            / "runs"
            / task
        ...
        if target.exists():
            print(f"SKIP [{i}/{len(jobs)}] {task} {arch} block={block} init={init_rep} {treatment}", flush=True)
            continue
        ...
        result = evaluate_one(
            model, data["val"], data["test"], task, arch, block, init_rep, args.batch_size, device
        )
        result.update(
            {
                "task": task,
        ...
        print(f"DONE [{i}/{len(jobs)}] {task} {arch} block={block} init={init_rep} {treatment}", flush=True)
        del model, saved
        if args.device == "cuda":
    ...
    if not args.smoke:
        ...
        if len(found) != expected:
            raise SystemExit(f"Incomplete Stage-F evaluation: {len(found)} / {expected}")
        atomic_json(
            out / "GRID_COMPLETE.json",
            {"planned": expected, "complete": len(found), "training_invoked": False},
        )
    print("Stage-F evaluation complete:", out, flush=True)
